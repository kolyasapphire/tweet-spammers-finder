...
const dataFetch = async (secrets: Secrets, continuation: string | null) => {
  ...
  const body = {
    url: `https://www.youtube.com/youtubei/v1/browse?key=${secrets.key}&prettyPrint=false`,
    options: {
      method: 'POST',
      body: proxyBody,
      headers: {
...
const deleteFetch = async (secrets: Secrets, setVideoId: string) => {
  ...
  const body = {
    url: `https://www.youtube.com/youtubei/v1/browse/edit_playlist?key=${secrets.key}&prettyPrint=false`,
    options: {
      method: 'POST',
      body: proxyBody,
      headers: {
